...
class HeapsSimpleView {
    ...
    inline public function circle( g: Sketcher
        ...
        circle( g.pen, p.x, p.y, radius, entitiesColor )
    }
    inline public function label( g: Graphics
                                , p: { x: Float, y: Float }
                                , t: String, font: Font
                                , fontSize: Int
                                , color: Int
    ...
    inline public function lineP( g: Graphics
                                , p0: { x: Float, y: Float }
                                , p1: { x: Float, y: Float }
                                ,   color: Int, alpha: Float, strength: Float ){
        g.lineStyle( strength, color );
        g.moveTo( p0.x, p0.y );
        g.lineTo( p1.x, p1.y );
    }
    public function drawVertex( g: Graphics, vertex : Vertex): Void {
    ...
    public function drawFace( g: Graphics, face: Face ) : Void {
        faceToEdgeIter.fromFace = face;
        var count = 0;
        var edge;
        // fills need resolve
        g.lineStyle( faceWidth, faceColor );
        var p: Point2D;
        while( true ){
    ...
    public function drawEdge( g: Graphics, edge : Edge ): Void {
    ...
    public function drawMesh( g: Graphics, mesh: Mesh ): Void {
        var all = mesh.getVerticesAndEdges();
        for (v in all.vertices) drawVertex( g, v );
        for (e in all.edges) drawEdge( g, e );
    }
    public function drawEntity( g: Graphics, entity: EntityAI ): Void {
        circle( g, entity, entity.radius, entitiesColor, entitiesAlpha );
    }
    public function drawEntities( g: Graphics, vEntities:Array<EntityAI> ): Void {
        for (i in 0...vEntities.length) drawEntity( g, vEntities[ i ] );
    }
    public function drawPath( g: Graphics, path:Array<Float>, cleanBefore:Bool = false): Void {
        if (path.length == 0) return;
        g.lineStyle( pathsWidth, pathsColor );
